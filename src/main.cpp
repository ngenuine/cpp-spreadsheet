...
void TestPositionAndStringConversion() {
    ...
    // все тесты проходят, но этот вообще ни в какую, потому что в моей программе,
    // AZZ1 получается, если по всем канонам остатки считать, поэтому перешел к другому
    // способу решить -- буду сразу насчитывать число в моей СС и сверять его с 701 в этом тесте
...
int main() {
    ...
    // auto to_str = [](const Position& p) { std::cout << p.ToString() << std::endl; }; 
    // auto form_str = [](std::string_view str) { std::cout << Position::FromString(str) << std::endl; }; 
    // to_str({0, 51}); // AZ1
    // to_str({0, 52}); // BA1
    // to_str({0, 701}); // ZZ1
    // to_str({0, 702}); // AAA1
