...
void TestFormulaReferences() {
    auto sheet = CreateSheet();
    auto evaluate = [&](std::string expr) {
        auto res = std::get<double>(ParseFormula(std::move(expr))->Evaluate(*sheet));
        return res;
    };
...
void MyFinalTest1() {
    ...
    sheet->SetCell("C1"_pos, "=A1+B2");

    const auto& cell = static_cast<Cell*>(sheet->GetCell("C1"_pos));
    ASSERT(cell->IsCacheInvalidated());
    ASSERT_EQUAL(std::get<double>(sheet->GetCell("C1"_pos)->GetValue()), 15);
    ASSERT(!cell->IsCacheInvalidated());
    sheet->SetCell("A1"_pos, "10");
    ASSERT(cell->IsCacheInvalidated());
    ASSERT_EQUAL(std::get<double>(sheet->GetCell("C1"_pos)->GetValue()), 20);
}
 ...
 */
    ...
    auto sheet = CreateSheet();
    auto get_ans = [&sheet](std::string index) {
        Position pos = Position::FromString(index);
        return std::get<double>(sheet->GetCell(pos)->GetValue());
    };

    auto is_inv = [&sheet](std::string index) {
        Position pos = Position::FromString(index);
        return static_cast<Cell*>(sheet->GetCell(pos))->IsCacheInvalidated();
    };

    sheet->SetCell("AA1"_pos, "=A1+A2");
    sheet->SetCell("A1"_pos, "=B1+B2");
    sheet->SetCell("A2"_pos, "=C1+C2");
    sheet->SetCell("B1"_pos, "=CA2");
    sheet->SetCell("CA2"_pos, "=X1+X2");
    ...
    sheet->SetCell("X1"_pos, "1");
    ASSERT_EQUAL(get_ans("AA1"s), 1.0);
    ...
    sheet->SetCell("X2"_pos, "=(10 + 3) * 2 - 1");
    ...
    ASSERT_EQUAL(get_ans("AA1"s), 26.0);
    ...
    ASSERT_EQUAL(get_ans("AA1"s), get_ans("CA2"s));
    sheet->SetCell("B2"_pos, "100");
    ASSERT(is_inv("AA1"s));
    ASSERT_EQUAL(get_ans("AA1"s), 126.0);
    ASSERT(!is_inv("AA1"s));
    sheet->SetCell("C1"_pos, "100");
    ...
    ASSERT_EQUAL(get_ans("AA1"s), 226.0);

    ASSERT(!is_inv("AA1"s));
    sheet->SetCell("C2"_pos, "=1000 + 1");
    ASSERT(is_inv("AA1"s));
    ASSERT_EQUAL(get_ans("AA1"s), 1227.0);
    ASSERT(!is_inv("AA1"s));

    bool caught = false;
    try {
        sheet->SetCell("X2"_pos, "=AA1+L1");
    } catch (const CircularDependencyException&) {
        caught = true;
    }

    ASSERT(caught);
    ASSERT_EQUAL(get_ans("X2"s), 25.0);
}
}  // namespace
...
int main() {
    ...
    RUN_TEST(tr, MyFinalTest1);
    RUN_TEST(tr, MyFinalTest2);

    std::cout << std::endl << "ALL TESTS OK"sv << std::endl;
}